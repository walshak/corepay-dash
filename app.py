...
from bitcoinlib.services.dashd import DashdClient
from bitcoinlib.services import authproxy
from bitcoinlib.mnemonic import Mnemonic
import os

#global vars
db_uri = 'mysql://corepay:18781875Core@localhost:3306/corepay_dash'
...
def create_wallet():
    ...
    w = Wallet.create(name, keys=passphrase, network='dash_testnet', db_uri=db_uri, owner=owner)
...
def get_balance():
    ...
    # scan for uxto's
    w.scan(network='dash_testnet')
    
    return jsonify({'balance': w.balance(network='dash_testnet')})
...
def get_address():
    ...
    w = Wallet(wallet_name,db_uri=db_uri)
    # get the wallet key
    key = w.get_key(network='dash_testnet')
...
def send_to():
    ...
    #open wallet
    w = Wallet(wallet_name,db_uri=db_uri)
    t = t = w.send_to(reciever_address, amount ,network='dash_testnet',offline=True)
